...
for (i=1:(N-1))
    ...
    elseif ((boundary=='per       ')&(n/2~=floor(n/2)))
        H1B(ictr,(n-2*(N-i)+2):n)=h(1:2*(N-i)-1);
        H1B(ictr,n)=H1B(ictr,n)+h(2*(N-i));
    else
...
for (i=1:(N-1+mod(n-2*N,2)))
    ...
    elseif ((boundary=='per       ')&(n/2~=floor(n/2)))
        H1B(ictr,(1:(2*i-mod(n-2*N,2)-1)))=h((2*(N-i)+1+mod(n-2*N,2)+1):end);
        H1B(ictr,n)=h((2*(N-i)+1+mod(n-2*N,2)));
    else
...
%%%%     row at the end.
%%%% 3. OR if boundary='per       ' then also keep extra row at the end.
%%%%
eps=1e-6;
keep_extra_flag=(sum(h)>eps)|min(boundary=='per       ');
if extend==0
    H1B=H1B((floor((N-1)/2)+1):(floor((N-1)/2)+floor(n/2)+mod(n,2)*(abs(keep_extra_flag))),:);
end;
end;
